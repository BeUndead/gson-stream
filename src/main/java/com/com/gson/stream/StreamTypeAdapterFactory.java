...
/**
 * Implementation of {@link TypeAdapterFactory} which returns {@link TypeAdapter TypeAdapters} for dealing with
 * {@link Stream Streams}.
 *
...
public final class StreamTypeAdapterFactory implements TypeAdapterFactory {
    ...
    /**
     * Implementation will return a {@link StreamTypeAdapter} if the provided {@link TypeToken type} is assignable to
     * {@link Stream}.  If the given {@code type} is generic, and the <strong>element</strong> type of the {@code
     * Stream} can be determined, a specific {@code StreamTypeAdapter} will be returned; otherwise one which uses
     * {@link Object} as the element type is instead returned.
     *
     * @param gson {@inheritDoc}
     * @param type {@inheritDoc}
     * @param <T>  {@inheritDoc}
     *
     * @return a {@code StreamTypeAdapter} if the given token is for a {@code Stream}; otherwise {@code null}
     */
